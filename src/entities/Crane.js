...
export class Crane {
    ...
    getAngleDegrees() {
        if (!this.constraint || !this.block) return this.lastAngleDegrees || 0;
        const dx = this.block.position.x - this.pivot.x;
        const dy = this.block.position.y - this.pivot.y;
        const degrees = Math.round(Math.abs(Math.atan2(dx, dy)) * (180 / Math.PI));
        this.lastAngleDegrees = degrees;
        return degrees;
    }

    drawAngleVisualization(ctx, game) {
        // Relocated to dedicated left-aligned UI display (#angle-display) in UIManager.js
    }
}
